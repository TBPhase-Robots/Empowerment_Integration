...
class UserInputs:
    ...
    def addMouseClick(self, time, realtime, mouse_button, screen_pos):
    ...
    def addAgentEvent(self, time, agent_id, pos, event_type):
        ...
        self.events_at_t[time]['position'].append(pos)
...
class Logger:
    ...
    def logPopulationStates(self, type, population, time):
        ...
        for agent in population:
            ...
            if type =='dog':
                empowerment_value = agent.empowerment
            else:
                empowerment_value = -1

            m_list[agent.id].update(time, agent.position, empowerment_value)
        return
    ...
    def logPopulations(self, populations, time):
        ...
        for pop in populations:
            for agent in pop:
                ids_at_t[idx] = agent.id
                positions_at_t[idx,:] = agent.position
                idx+=1
